...
enum listtype {
	LT_GENERIC, LT_USER, LT_GROUP, LT_TTY, LT_PGRP, LT_JID, LT_SID, LT_CLASS
};
struct list {
...
int main( int argc, char **argv ) {
	...
	if ( strcmp( getprogname(), "pgrep" ) == 0 ) {
		...
		if ( argc > 1 && p[0] == '-' ) {
			...
			if ( *q == '\0' ) {
				...
				for ( i = 1; i < NSIG; i++ )
					if ( strcasecmp( sys_signame[i], p ) == 0 ) break;
				if ( i != NSIG ) {
					signum = i;
					argv++;
					argc--;
	...
	if ( pidfile != NULL ) pidfromfile = takepid( pidfile, pidfilelock );
		...
		if ( pidfilelock ) {
			errx( STATUS_ERROR, "Option -L doesn't make sense without -F" );
		}
		pidfromfile = -1;
	}
	mypid = getpid();
	kd = kvm_openfiles( execf, coref, NULL, O_RDONLY, buf );
	if ( kd == NULL ) errx( STATUS_ERROR, "Cannot open kernel files (%s)", buf );
	plist = kvm_getprocs( kd, KERN_PROC_PROC, 0, &nproc );
	if ( plist == NULL ) {
		errx( STATUS_ERROR, "Cannot get process list (%s)", kvm_geterr( kd ) );
	}
	if ( ( selected = malloc( nproc ) ) == NULL ) {
		err( STATUS_ERROR, "Cannot allocate memory for %d processes", nproc );
	}
	memset( selected, 0, nproc );
	for ( ; *argv != NULL; argv++ ) {
		if ( ( rv = regcomp( &reg, *argv, cflags ) ) != 0 ) {
			regerror( rv, &reg, buf, sizeof( buf ) );
			errx( STATUS_BADUSAGE, "Cannot compile regular expression `%s' (%s)", *argv, buf );
		}
		for ( i = 0, kp = plist; i < nproc; i++, kp++ ) {
			if ( PSKIP( kp ) ) {
				if ( debug_opt > 0 ) fprintf( stderr, "* Skipped %5d %3d %s\n", kp->ki_pid, kp->ki_uid, kp->ki_comm );
				continue;
			}
			if ( matchargs && ( pargv = kvm_getargv( kd, kp, 0 ) ) != NULL ) {
				jsz = 0;
				while ( jsz < sizeof( buf ) && *pargv != NULL ) {
					jsz += snprintf( buf + jsz, sizeof( buf ) - jsz, pargv[1] != NULL ? "%s " : "%s", pargv[0] );
					pargv++;
				}
				mstr = buf;
			} else mstr = kp->ki_comm;
			rv = regexec( &reg, mstr, 1, &regmatch, 0 );
			if ( rv == 0 ) {
				if ( fullmatch ) {
					if ( regmatch.rm_so == 0 && regmatch.rm_eo == (off_t) strlen( mstr ) ) selected[i] = 1;
				} else selected[i] = 1;
			} else if ( rv != REG_NOMATCH ) {
				regerror( rv, &reg, buf, sizeof( buf ) );
				errx( STATUS_ERROR, "Regular expression evaluation error (%s)", buf );
			}
			if ( debug_opt > 1 ) {
				const char *rv_res = "NoMatch";
				if ( selected[i] ) rv_res = "Matched";
				fprintf( stderr, "* %s %5d %3d %s\n", rv_res, kp->ki_pid, kp->ki_uid, mstr );
			}
		}
		regfree( &reg );
	}
	for ( i = 0, kp = plist; i < nproc; i++, kp++ ) {
		if ( PSKIP( kp ) ) continue;
		if ( pidfromfile >= 0 && kp->ki_pid != pidfromfile ) {
			selected[i] = 0;
			continue;
		}
		SLIST_FOREACH( li, &ruidlist, li_chain )
		if ( kp->ki_ruid == (uid_t) li->li_number ) break;
		if ( SLIST_FIRST( &ruidlist ) != NULL && li == NULL ) {
			selected[i] = 0;
			continue;
		}
		SLIST_FOREACH( li, &rgidlist, li_chain )
		if ( kp->ki_rgid == (gid_t) li->li_number ) break;
		if ( SLIST_FIRST( &rgidlist ) != NULL && li == NULL ) {
			selected[i] = 0;
			continue;
		}
		SLIST_FOREACH( li, &euidlist, li_chain )
		if ( kp->ki_uid == (uid_t) li->li_number ) break;
		if ( SLIST_FIRST( &euidlist ) != NULL && li == NULL ) {
			selected[i] = 0;
			continue;
		}
		SLIST_FOREACH( li, &ppidlist, li_chain )
		if ( kp->ki_ppid == (pid_t) li->li_number ) break;
		if ( SLIST_FIRST( &ppidlist ) != NULL && li == NULL ) {
			selected[i] = 0;
			continue;
		}
		SLIST_FOREACH( li, &pgrplist, li_chain )
		if ( kp->ki_pgid == (pid_t) li->li_number ) break;
		...
		{
			if ( li->li_number == -1 && ( kp->ki_flag & P_CONTROLT ) == 0 ) break;
			if ( kp->ki_tdev == (dev_t) li->li_number ) break;
		}
		...
		if ( kp->ki_sid == (pid_t) li->li_number ) break;
		...
		{
			if ( kp->ki_jid == (int) li->li_number ) break;
			if ( kp->ki_jid > 0 && li->li_number == -1 ) break;
		}
		...
		{
			if ( ( kp->ki_flag & P_SYSTEM ) == 0 && strcmp( kp->ki_loginclass, li->li_name ) == 0 ) break;
		}
		if ( SLIST_FIRST( &classlist ) != NULL && li == NULL ) {
			selected[i] = 0;
			continue;
		}
		if ( argc == 0 ) selected[i] = 1;
	}
	if ( !ancestors ) {
		pid = mypid;
		while ( pid ) {
	...
	if ( newest || oldest ) {
		...
		memset( selected, 0, nproc );
		if ( bestidx != -1 ) selected[bestidx] = 1;
	}
	did_action = 0;
...
static void show_process( const struct kinfo_proc *kp ) {
	...
	if ( ( longfmt || !pgrep ) && matchargs && ( argv = kvm_getargv( kd, kp, 0 ) ) != NULL ) {
		printf( "%d ", (int) kp->ki_pid );
		for ( ; *argv != NULL; argv++ ) {
			printf( "%s", *argv );
			if ( argv[1] != NULL ) putchar( ' ' );
		}
	} else if ( longfmt || !pgrep ) printf( "%d %s", (int) kp->ki_pid, kp->ki_comm );
	else printf( "%d", (int) kp->ki_pid );
}
static int killact( const struct kinfo_proc *kp ) {
	int ch, first;
	if ( interactive ) {
		printf( "kill " );
		show_process( kp );
	...
	if ( kill( kp->ki_pid, signum ) == -1 ) {
		if ( errno != ESRCH ) warn( "signalling pid %d", (int) kp->ki_pid );
		return ( 0 );
	}
	return ( 1 );
}
static int grepact( const struct kinfo_proc *kp ) {
	show_process( kp );
	if ( !quiet ) printf( "%s", delim );
	return ( 1 );
}
static void makelist( struct listhead *head, enum listtype type, char *src ) {
	struct list *li;
	struct passwd *pw;
	struct group *gr;
	struct stat st;
	...
	while ( ( sp = strsep( &src, "," ) ) != NULL ) {
		...
		empty = 0;
		if ( type != LT_CLASS ) li->li_number = (uid_t) strtol( sp, &ep, 0 );
		if ( type != LT_CLASS && *ep == '\0' ) {
			switch ( type ) {
				case LT_PGRP:
					if ( li->li_number == 0 ) li->li_number = getpgrp();
					break;
				case LT_SID:
					if ( li->li_number == 0 ) li->li_number = getsid( mypid );
					break;
				case LT_JID:
					if ( li->li_number < 0 ) errx( STATUS_BADUSAGE, "Negative jail ID `%s'", sp );
					if ( li->li_number == 0 ) li->li_number = -1;
					break;
				case LT_TTY:
					if ( li->li_number < 0 ) errx( STATUS_BADUSAGE, "Negative /dev/pts tty `%s'", sp );
					snprintf(buf, sizeof(buf), _PATH_DEV "pts/%s",
							sp);
					if ( stat( buf, &st ) != -1 ) goto foundtty;
					if (errno == ENOENT)
		...
		switch ( type ) {
			case LT_USER:
				if ( ( pw = getpwnam( sp ) ) == NULL ) errx( STATUS_BADUSAGE, "Unknown user `%s'", sp );
				li->li_number = pw->pw_uid;
				break;
			case LT_GROUP:
				if ( ( gr = getgrnam( sp ) ) == NULL ) errx( STATUS_BADUSAGE, "Unknown group `%s'", sp );
			...
			case LT_TTY:
				...
				if ( stat( buf, &st ) != -1 ) goto foundtty;
				if ( errno == ENOENT ) errx( STATUS_BADUSAGE, "No such tty: `%s'", sp );
				err( STATUS_ERROR, "Cannot access `%s'", sp );
				foundtty: if ( ( st.st_mode & S_IFCHR ) == 0 ) errx( STATUS_BADUSAGE, "Not a tty: `%s'", sp );
				li->li_number = st.st_rdev;
				break;
			case LT_JID:
				if ( strcmp( sp, "none" ) == 0 ) li->li_number = 0;
				else if ( strcmp( sp, "any" ) == 0 ) li->li_number = -1;
				else if ( *ep != '\0' ) errx( STATUS_BADUSAGE, "Invalid jail ID `%s'", sp );
				break;
			case LT_CLASS:
				li->li_number = -1;
...
static int takepid( const char *pidfile, int pidfilelock ) {
	...
	long rval;
	fh = fopen( pidfile, "r" );
	if ( fh == NULL ) err( STATUS_ERROR, "Cannot open pidfile `%s'", pidfile );
	if ( pidfilelock ) {
		if ( flock( fileno( fh ), LOCK_EX | LOCK_NB ) == 0 ) {
			...
			if ( errno != EWOULDBLOCK ) {
				errx( STATUS_ERROR, "Error while locking file '%s'", pidfile );
			}
		}
	}
	...
	if ( *endp != '\0' && !isspace( (unsigned char) *endp ) ) errx( STATUS_ERROR, "Invalid pid in file `%s'", pidfile );
	else if ( rval < MIN_PID || rval > MAX_PID ) errx( STATUS_ERROR, "Invalid pid in file `%s'", pidfile );
	return ( rval );
}
