...
struct nc_protos {
		...
		int af;
		int sotype;
} nc_protos[] = { { "udp", AF_INET, SOCK_DGRAM }, { "tcp", AF_INET, SOCK_STREAM }, { "udp6", AF_INET6, SOCK_DGRAM }, { "tcp6", AF_INET6, SOCK_STREAM }, { NULL, 0, 0 } };
struct nfhret {
		u_long stat;
		long vers;
		long auth;
...
enum tryret {
	TRYRET_SUCCESS, TRYRET_TIMEOUT, TRYRET_REMOTEERR, TRYRET_LOCALERR
};
...
static enum tryret nfs_tryproto( struct addrinfo *ai, char *hostp, char *spec, char **errstr, struct iovec **iov, int *iovlen );
...
int main( int argc, char *argv[] ) {
	...
	if ( fstype == NULL ) errx( EX_USAGE, "argv[0] must end in _fstype" );
	++fstype;
	while ( ( c = getopt( argc, argv, "23a:bcdD:g:I:iLlNo:PR:r:sTt:w:x:U" ) ) != -1 )
		switch ( c ) {
			case '2':
				mountmode = V2;
				break;
			...
			case 'o': {
				...
				while ( opt ) {
					...
					if ( strcmp( opt, "bg" ) == 0 ) {
						...
						fstype = "nfs";
						nfsproto = IPPROTO_TCP;
						if ( portspec == NULL ) portspec = "2049";
						...
						if ( secflavor < 0 ) {
							errx( 1, "illegal sec value -- %s", val );
						}
	...
	spec = *argv++;
	name = *argv;
	if ( retrycnt == -1 ) retrycnt = 0;
	if ( strcmp( fstype, "nfs" ) == 0 ) {
		if ( modfind( "nfscl" ) < 0 ) {
			if ( kldload( "nfscl" ) < 0 || modfind( "nfscl" ) < 0 ) errx( 1, "nfscl is not available" );
		}
	}
	if ( gssname != NULL ) {
		if ( strchr( gssname, '@' ) == NULL && gethostname( hostname, MAXHOSTNAMELEN ) == 0 ) {
			snprintf( gssn, sizeof( gssn ), "%s@%s", gssname, hostname );
			gssname = gssn;
		}
		build_iovec( &iov, &iovlen, "gssname", gssname, strlen( gssname ) + 1 );
	}
	if ( !getnfsargs( spec, &iov, &iovlen ) ) exit( 1 );
	if ( checkpath( name, mntpath ) != 0 ) err( 1, "%s", mntpath );
	build_iovec( &iov, &iovlen, "fstype", fstype, ( size_t ) - 1 );
	build_iovec( &iov, &iovlen, "fspath", mntpath, ( size_t ) - 1 );
	build_iovec( &iov, &iovlen, "errmsg", errmsg, sizeof( errmsg ) );
	osversion = getosreldate();
	if ( osversion >= 702100 ) {
		if ( nmount( iov, iovlen, 0 ) ) err( 1, "%s, %s", mntpath, errmsg );
	} else {
		if ( fallback_mount( iov, iovlen ) ) err( 1, "%s, %s", mntpath, errmsg );
	}
	exit( 0 );
}
static int findopt( struct iovec *iov, int iovlen, const char *name, char **valuep, int *lenp ) {
	int i;
	for ( i = 0; i < iovlen / 2; i++, iov += 2 ) {
		if ( strcmp( name, iov[0].iov_base ) == 0 ) {
			if ( valuep ) *valuep = iov[1].iov_base;
			if ( lenp ) *lenp = iov[1].iov_len;
			return ( 0 );
		}
	}
	return ( ENOENT );
}
static void copyopt( struct iovec **newiov, int *newiovlen, struct iovec *iov, int iovlen, const char *name ) {
	char *value;
	int len;
	if ( findopt( iov, iovlen, name, &value, &len ) == 0 ) build_iovec( newiov, newiovlen, name, value, len );
}
static int fallback_mount( struct iovec *iov, int iovlen ) {
...
static int sec_name_to_num( char *sec ) {
if ( !strcmp( sec, "krb5" ) ) return ( RPCSEC_GSS_KRB5 );
if ( !strcmp( sec, "krb5i" ) ) return ( RPCSEC_GSS_KRB5I );
if ( !strcmp( sec, "krb5p" ) ) return ( RPCSEC_GSS_KRB5P );
if ( !strcmp( sec, "sys" ) ) return ( AUTH_SYS );
return ( -1 );
}
static char *
...
static int getnfsargs( char *spec, struct iovec **iov, int *iovlen ) {
...
if ( *spec == '[' && ( delimp = strchr( spec + 1, ']' ) ) != NULL && *( delimp + 1 ) == ':' ) {
	hostp = spec + 1;
	spec = delimp + 2;
	have_bracket = 1;
...
*delimp = '\0';
for ( speclen = strlen( spec ); speclen > 1 && spec[speclen - 1] == '/'; speclen-- )
	spec[speclen - 1] = '\0';
...
if ( *hostp != '\0' ) {
	len = strlen( hostp );
	offset = 0;
	if ( have_bracket ) nam[offset++] = '[';
	memmove( nam + offset, hostp, len );
	if ( have_bracket ) nam[len + offset++] = ']';
	nam[len + offset++] = ':';
...
if ( nfsproto == IPPROTO_TCP ) hints.ai_socktype = SOCK_STREAM;
else if ( nfsproto == IPPROTO_UDP ) hints.ai_socktype = SOCK_DGRAM;
if ( getaddrinfo( hostp, portspec, &hints, &ai_nfs ) != 0 ) {
	hints.ai_flags = AI_CANONNAME;
	if ( ( ecode = getaddrinfo( hostp, portspec, &hints, &ai_nfs ) ) != 0 ) {
		if ( portspec == NULL ) errx( 1, "%s: %s", hostp, gai_strerror( ecode ) );
		else errx( 1, "%s:%s: %s", hostp, portspec, gai_strerror( ecode ) );
		return ( 0 );
	}
	if ( got_principal == 0 && secflavor >= 0 && secflavor != AUTH_SYS && ai_nfs->ai_canonname != NULL ) {
		snprintf( pname, sizeof( pname ), "nfs@%s", ai_nfs->ai_canonname );
		build_iovec( iov, iovlen, "principal", pname, strlen( pname ) + 1 );
	}
}
ret = TRYRET_LOCALERR;
for ( ;; ) {
	...
	for ( ai = ai_nfs; ai != NULL; ai = ai->ai_next ) {
		if ( ( ai->ai_family == AF_INET6 ) && ( opflags & OF_NOINET6 ) ) continue;
		if ( ( ai->ai_family == AF_INET ) && ( opflags & OF_NOINET4 ) ) continue;
		ret = nfs_tryproto( ai, hostp, spec, &errstr, iov, iovlen );
		if ( ret == TRYRET_SUCCESS ) break;
		if ( ret != TRYRET_LOCALERR ) remoteerr = 1;
		if ( ( opflags & ISBGRND ) == 0 ) fprintf( stderr, "%s\n", errstr );
	}
	if ( ret == TRYRET_SUCCESS ) break;
	if ( !remoteerr ) exit( 1 );
	if ( retrycnt != 0 && --retrycnt == 0 ) exit( 1 );
...
if ( !add_mtab( hostp, spec ) ) warnx( "can't update %s for %s:%s", PATH_MOUNTTAB, hostp, spec );
return ( 1 );
}
static enum tryret nfs_tryproto( struct addrinfo *ai, char *hostp, char *spec, char **errstr, struct iovec **iov, int *iovlen ) {
...
errbuf[0] = '\0';
*errstr = errbuf;
if ( nfsproto == IPPROTO_TCP ) sotype = SOCK_STREAM;
else if ( nfsproto == IPPROTO_UDP ) sotype = SOCK_DGRAM;
if ( ( netid = netidbytype( ai->ai_family, sotype ) ) == NULL ) {
	snprintf( errbuf, sizeof errbuf, "af %d sotype %d not supported", ai->ai_family, sotype );
...
if ( mnttcp_ok ) {
	...
	if ( ( netid_mnt = netidbytype( ai->ai_family, SOCK_DGRAM ) ) == NULL ) {
		snprintf( errbuf, sizeof errbuf, "af %d sotype SOCK_DGRAM not supported", ai->ai_family );
		return ( TRYRET_LOCALERR );
	}
	if ( ( nconf_mnt = getnetconf_cached( netid_mnt ) ) == NULL ) {
		snprintf( errbuf, sizeof errbuf, "%s: %s", netid_mnt, nc_sperror() );
		return ( TRYRET_LOCALERR );
	}
}
tryagain: if ( trymntmode == V4 ) {
	nfsvers = 4;
...
if ( portspec != NULL ) {
	...
	if ( !rpcb_getaddr( NFS_PROGRAM, nfsvers, nconf, &nfs_nb, hostp ) ) {
		if ( rpc_createerr.cf_stat == RPC_PROGVERSMISMATCH && trymntmode == ANY ) {
			trymntmode = V2;
			goto tryagain;
		}
		snprintf( errbuf, sizeof errbuf, "[%s] %s:%s: %s", netid, hostp, spec, clnt_spcreateerror( "RPCPROG_NFS" ) );
		return ( returncode( rpc_createerr.cf_stat, &rpc_createerr.cf_error ) );
	}
}
clp = clnt_tli_create( RPC_ANYFD, nconf, &nfs_nb, NFS_PROGRAM, nfsvers, 0, 0 );
if ( clp == NULL ) {
	snprintf( errbuf, sizeof errbuf, "[%s] %s:%s: %s", netid, hostp, spec, clnt_spcreateerror( "nfsd: RPCPROG_NFS" ) );
	return ( returncode( rpc_createerr.cf_stat, &rpc_createerr.cf_error ) );
}
if ( sotype == SOCK_DGRAM && noconn == 0 ) {
	...
	if ( !clnt_control( clp, CLSET_CONNECT, (char *) &doconnect ) ) {
		clnt_destroy( clp );
		snprintf( errbuf, sizeof errbuf, "[%s] %s:%s: CLSET_CONNECT failed", netid, hostp, spec );
		return ( TRYRET_LOCALERR );
	}
}
try.tv_sec = 10;
...
if ( stat != RPC_SUCCESS ) {
	...
	snprintf( errbuf, sizeof errbuf, "[%s] %s:%s: %s", netid, hostp, spec, clnt_sperror( clp, "NFSPROC_NULL" ) );
	clnt_destroy( clp );
...
if ( trymntmode == V4 ) {
	...
	if ( secname != NULL ) build_iovec( iov, iovlen, "sec", secname, ( size_t ) - 1 );
...
if ( clp == NULL ) {
	snprintf( errbuf, sizeof errbuf, "[%s] %s:%s: %s", netid_mnt, hostp, spec, clnt_spcreateerror( "RPCMNT: clnt_create" ) );
	return ( returncode( rpc_createerr.cf_stat, &rpc_createerr.cf_error ) );
}
...
if ( stat != RPC_SUCCESS ) {
	...
	clnt_geterr( clp, &rpcerr );
	snprintf( errbuf, sizeof errbuf, "[%s] %s:%s: %s", netid_mnt, hostp, spec, clnt_sperror( clp, "RPCPROG_MNT" ) );
	clnt_destroy( clp );
	return ( returncode( stat, &rpcerr ) );
}
clnt_destroy( clp );
if ( nfhret.stat != 0 ) {
	snprintf( errbuf, sizeof errbuf, "[%s] %s:%s: %s", netid_mnt, hostp, spec, strerror( nfhret.stat ) );
...
if ( secname ) build_iovec( iov, iovlen, "sec", secname, ( size_t ) - 1 );
if ( nfsvers == 3 ) build_iovec( iov, iovlen, "nfsv3", NULL, 0 );
return ( TRYRET_SUCCESS );
}
static enum tryret returncode( enum clnt_stat stat, struct rpc_err *rpcerr ) {
switch ( stat ) {
...
for ( p = nc_protos; p->netid != NULL; p++ ) {
	if ( af != p->af || sotype != p->sotype ) continue;
	return ( p->netid );
}
return ( NULL );
}
...
for ( p = head; p != NULL; p = p->next )
	if ( strcmp( netid, p->nconf->nc_netid ) == 0 ) return ( p->nconf );
if ( ( nconf = getnetconfigent( netid ) ) == NULL ) return ( NULL );
if ( ( p = malloc( sizeof( *p ) ) ) == NULL ) err( 1, "malloc" );
p->nconf = nconf;
p->next = head;
head = p;
return ( p->nconf );
...
if ( !xdr_u_long( xdrsp, &np->stat ) ) return ( 0 );
if ( np->stat ) return ( 1 );
switch ( np->vers ) {
	case 1:
	...
	case 3:
		if ( !xdr_long( xdrsp, &np->fhsize ) ) return ( 0 );
		...
		for ( i = 0; i < authcnt; i++ ) {
			if ( !xdr_long( xdrsp, &auth ) ) return ( 0 );
			if ( np->auth == -1 ) {
				np->auth = auth;
				authfnd++;
		...
		if ( authcnt == 0 && np->auth == -1 ) np->auth = AUTH_SYS;
		if ( !authfnd && ( authcnt > 0 || np->auth != AUTH_SYS ) ) np->stat = EAUTH;
		return ( 1 );
};
return ( 0 );
}
static void usage( void ) {
(void) fprintf( stderr, "%s\n%s\n%s\n%s\n", "usage: mount_nfs [-23bcdiLlNPsTU] [-a maxreadahead] [-D deadthresh]", "                 [-g maxgroups] [-I readdirsize] [-o options] [-R retrycnt]", "                 [-r readsize] [-t timeout] [-w writesize] [-x retrans]", "                 rhost:path node" );
exit( 1 );
}
