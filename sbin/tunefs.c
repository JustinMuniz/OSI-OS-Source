...
int main( int argc, char *argv[] ) {
	...
	active = 0;
	found_arg = 0;
	while ( ( ch = getopt( argc, argv, "Aa:e:f:j:J:k:L:l:m:N:n:o:ps:S:t:" ) ) != -1 )
		switch ( ch ) {
			case 'A':
				found_arg = 1;
				Aflag++;
			...
			case 'a':
				...
				if ( strcmp( avalue, "enable" ) && strcmp( avalue, "disable" ) ) {
					errx( 10, "bad %s (options are %s)", name, "`enable' or `disable'" );
				}
				aflag = 1;
				break;
			case 'e':
				...
				evalue = atoi( optarg );
				if ( evalue < 1 ) errx( 10, "%s must be >= 1 (was %s)", name, optarg );
				eflag = 1;
				break;
			case 'f':
				found_arg = 1;
				...
				fvalue = atoi( optarg );
				if ( fvalue < 1 ) errx( 10, "%s must be >= 1 (was %s)", name, optarg );
				fflag = 1;
				break;
			case 'j':
				found_arg = 1;
				...
				jvalue = optarg;
				if ( strcmp( jvalue, "enable" ) && strcmp( jvalue, "disable" ) ) {
					errx( 10, "bad %s (options are %s)", name, "`enable' or `disable'" );
				}
				jflag = 1;
				break;
			...
			case 'k':
				...
				kvalue = atoi( optarg );
				if ( kvalue < 0 ) errx( 10, "bad %s (%s)", name, optarg );
				kflag = 1;
				break;
			case 'L':
				found_arg = 1;
				...
				while ( isalnum( Lvalue[++i] ) )
					;
				if ( Lvalue[i] != '\0' ) {
					errx( 10, "bad %s. Valid characters are alphanumerics.", name );
				}
				if ( strlen( Lvalue ) >= MAXVOLLEN ) {
					errx( 10, "bad %s. Length is longer than %d.", name, MAXVOLLEN - 1 );
				}
				Lflag = 1;
				break;
			case 'l':
			...
			case 'm':
				...
				mvalue = atoi( optarg );
				if ( mvalue < 0 || mvalue > 99 ) errx( 10, "bad %s (%s)", name, optarg );
				mflag = 1;
				break;
			case 'N':
				found_arg = 1;
			...
			case 'n':
				...
				name = "soft updates";
				nvalue = optarg;
				if ( strcmp( nvalue, "enable" ) != 0 && strcmp( nvalue, "disable" ) != 0 ) {
					errx( 10, "bad %s (options are %s)", name, "`enable' or `disable'" );
				}
				nflag = 1;
				break;
			case 'o':
				...
				if ( strcmp( optarg, "space" ) == 0 ) ovalue = FS_OPTSPACE;
				else if ( strcmp( optarg, "time" ) == 0 ) ovalue = FS_OPTTIME;
				else errx( 10, "bad %s (options are `space' or `time')", name );
				oflag = 1;
				break;
			case 'p':
				found_arg = 1;
			...
			case 's':
				...
				svalue = atoi( optarg );
				if ( svalue < 1 ) errx( 10, "%s must be >= 1 (was %s)", name, optarg );
				sflag = 1;
				break;
			case 'S':
				found_arg = 1;
				...
				Svalue = atoi( optarg );
				if ( Svalue < SUJ_MIN ) errx( 10, "%s must be >= %d (was %s)", name, SUJ_MIN, optarg );
				break;
			case 't':
				found_arg = 1;
				name = "trim";
				tvalue = optarg;
				if ( strcmp( tvalue, "enable" ) != 0 && strcmp( tvalue, "disable" ) != 0 ) {
					errx( 10, "bad %s (options are %s)", name, "`enable' or `disable'" );
				}
				tflag = 1;
				break;
	...
	argc -= optind;
	argv += optind;
	if ( found_arg == 0 || argc != 1 ) usage();
	on = special = argv[0];
	if ( ufs_disk_fillout( &disk, special ) == -1 ) goto err;
	if ( disk.d_name != special ) {
		if ( statfs( special, &stfs ) != 0 ) warn( "Can't stat %s", special );
		if ( strcmp( special, stfs.f_mntonname ) == 0 ) active = 1;
	}
	if ( pflag ) {
		printfs();
		exit( 0 );
	...
	if ( eflag ) {
		name = "maximum blocks per file in a cylinder group";
		if ( sblock.fs_maxbpg == evalue ) warnx( "%s remains unchanged as %d", name, evalue );
		else {
			warnx( "%s changes from %d to %d", name, sblock.fs_maxbpg, evalue );
	...
	if ( fflag ) {
		...
		if ( sblock.fs_avgfilesize == (unsigned) fvalue ) {
			...
			warnx( "%s changes from %d to %d", name, sblock.fs_avgfilesize, fvalue );
	...
	if ( jflag ) {
		...
		if ( strcmp( jvalue, "enable" ) == 0 ) {
			if ( ( sblock.fs_flags & ( FS_DOSOFTDEP | FS_SUJ ) ) == ( FS_DOSOFTDEP | FS_SUJ ) ) {
				warnx( "%s remains unchanged as enabled", name );
			} else if ( sblock.fs_clean == 0 ) {
				warnx( "%s cannot be enabled until fsck is run", name );
			...
			if ( ( ~sblock.fs_flags & FS_SUJ ) == FS_SUJ ) {
				...
				sblock.fs_sujfree = 0;
				warnx( "%s cleared but soft updates still set.", name );
	...
	if ( kflag ) {
		name = "space to hold for metadata blocks";
		if ( sblock.fs_metaspace == kvalue ) warnx( "%s remains unchanged as %d", name, kvalue );
		else {
			...
			warnx( "%s changes from %jd to %d", name, sblock.fs_metaspace, kvalue );
	...
	if ( mflag ) {
		name = "minimum percentage of free space";
		if ( sblock.fs_minfree == mvalue ) warnx( "%s remains unchanged as %d%%", name, mvalue );
		else {
			warnx( "%s changes from %d%% to %d%%", name, sblock.fs_minfree, mvalue );
			sblock.fs_minfree = mvalue;
			if ( mvalue >= MINFREE && sblock.fs_optim == FS_OPTSPACE ) warnx( OPTWARN, "time", ">=", MINFREE );
			if ( mvalue < MINFREE && sblock.fs_optim == FS_OPTTIME ) warnx( OPTWARN, "space", "<", MINFREE );
		}
	}
	if ( Nflag ) {
		name = "NFSv4 ACLs";
	...
	if ( nflag ) {
		name = "soft updates";
		if ( strcmp( nvalue, "enable" ) == 0 ) {
			if ( sblock.fs_flags & FS_DOSOFTDEP ) warnx( "%s remains unchanged as enabled", name );
			else if ( sblock.fs_clean == 0 ) {
				warnx( "%s cannot be enabled until fsck is run", name );
			} else {
			...
			if ( ( ~sblock.fs_flags & FS_DOSOFTDEP ) == FS_DOSOFTDEP ) warnx( "%s remains unchanged as disabled", name );
			else {
				sblock.fs_flags &= ~FS_DOSOFTDEP;
				warnx( "%s cleared", name );
			}
	...
	if ( oflag ) {
		...
		chg[FS_OPTTIME] = "time";
		if ( sblock.fs_optim == ovalue ) warnx( "%s remains unchanged as %s", name, chg[ovalue] );
		else {
			warnx( "%s changes from %s to %s", name, chg[sblock.fs_optim], chg[ovalue] );
			sblock.fs_optim = ovalue;
			if ( sblock.fs_minfree >= MINFREE && ovalue == FS_OPTSPACE ) warnx( OPTWARN, "time", ">=", MINFREE );
			if ( sblock.fs_minfree < MINFREE && ovalue == FS_OPTTIME ) warnx( OPTWARN, "space", "<", MINFREE );
		}
	}
	if ( sflag ) {
		name = "expected number of files per directory";
		if ( sblock.fs_avgfpdir == (unsigned) svalue ) {
			...
			warnx( "%s changes from %d to %d", name, sblock.fs_avgfpdir, svalue );
	...
	if ( tflag ) {
		...
		if ( strcmp( tvalue, "enable" ) == 0 ) {
			if ( sblock.fs_flags & FS_TRIM ) warnx( "%s remains unchanged as enabled", name );
			else {
				sblock.fs_flags |= FS_TRIM;
				warnx( "%s set", name );
			}
		} else if ( strcmp( tvalue, "disable" ) == 0 ) {
			if ( ( ~sblock.fs_flags & FS_TRIM ) == FS_TRIM ) warnx( "%s remains unchanged as disabled", name );
			else {
				sblock.fs_flags &= ~FS_TRIM;
				warnx( "%s cleared", name );
			}
	...
	if ( sbwrite( &disk, Aflag ) == -1 ) goto err;
	ufs_disk_close( &disk );
	if ( active ) {
		bzero( &args, sizeof( args ) );
		if ( mount( "ufs", on, stfs.f_flags | MNT_UPDATE | MNT_RELOAD, &args ) < 0 ) err( 9, "%s: reload", special );
		warnx( "file system reloaded" );
	}
	exit( 0 );
	err: if ( disk.d_error != NULL ) errx( 11, "%s: %s", special, disk.d_error );
	else err( 12, "%s", special );
}
static void sbdirty( void ) {
...
static ino_t dir_search( ufs2_daddr_t blk, int bytes ) {
	char block[MAXBSIZE];
	struct direct *dp;
	int off;
	...
	for ( off = 0; off < bytes; off += dp->d_reclen ) {
		...
		if ( dp->d_namlen != strlen( SUJ_FILE ) ) continue;
		if ( bcmp( dp->d_name, SUJ_FILE, dp->d_namlen ) != 0 ) continue;
		return ( dp->d_ino );
	}
	return ( 0 );
}
...
static void dir_clear_block( const char *block, off_t off ) {
	struct direct *dp;
...
static int dir_insert( ufs2_daddr_t blk, off_t off, ino_t ino ) {
	struct direct *dp;
	char block[MAXBSIZE];
...
static int indir_fill( ufs2_daddr_t blk, int level, int *resid ) {
	...
	for ( i = 0; i < NINDIR( &sblock ) && *resid != 0; i++ ) {
		...
		if ( nblk <= 0 ) return ( -1 );
		cnt++;
		if ( sblock.fs_magic == FS_UFS1_MAGIC ) *bap1++ = nblk;
		else *bap2++ = nblk;
		if ( level != 0 ) {
...
static void journal_clear( void ) {
	...
	dp2 = ip;
	dp1 = ip;
	if ( sblock.fs_magic == FS_UFS1_MAGIC ) dp1->di_flags = 0;
	else dp2->di_flags = 0;
	if ( putino( &disk ) < 0 ) {
...
static void usage( void ) {
	fprintf( stderr, "%s\n%s\n%s\n%s\n%s\n%s\n", "usage: tunefs [-A] [-a enable | disable] [-e maxbpg] [-f avgfilesize]", "              [-J enable | disable] [-j enable | disable] [-k metaspace]", "              [-L volname] [-l enable | disable] [-m minfree]", "              [-N enable | disable] [-n enable | disable]", "              [-o space | time] [-p] [-s avgfpdir] [-t enable | disable]", "              special | filesystem" );
	exit( 2 );
}
static void printfs( void ) {
	warnx( "POSIX.1e ACLs: (-a)                                %s", ( sblock.fs_flags & FS_ACLS ) ? "enabled" : "disabled" );
	warnx( "NFSv4 ACLs: (-N)                                   %s", ( sblock.fs_flags & FS_NFS4ACLS ) ? "enabled" : "disabled" );
	warnx( "MAC multilabel: (-l)                               %s", ( sblock.fs_flags & FS_MULTILABEL ) ? "enabled" : "disabled" );
	warnx( "soft updates: (-n)                                 %s", ( sblock.fs_flags & FS_DOSOFTDEP ) ? "enabled" : "disabled" );
	warnx( "soft update journaling: (-j)                       %s", ( sblock.fs_flags & FS_SUJ ) ? "enabled" : "disabled" );
	warnx( "gjournal: (-J)                                     %s", ( sblock.fs_flags & FS_GJOURNAL ) ? "enabled" : "disabled" );
	warnx( "trim: (-t)                                         %s", ( sblock.fs_flags & FS_TRIM ) ? "enabled" : "disabled" );
	...
	if ( sblock.fs_minfree < MINFREE &&
	sblock.fs_optim == FS_OPTTIME ) warnx( OPTWARN, "space", "<", MINFREE );
