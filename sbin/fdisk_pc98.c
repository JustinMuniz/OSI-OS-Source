...
int main( int argc, char *argv[] ) {
	...
	if ( argc == 0 ) {
		...
		if ( stat( argv[0], &sb ) == 0 ) {
			...
			if ( disk == NULL ) errx( 1, "out of memory" );
		} else {
			disk = argv[0];
		}
	}
	...
	if ( s_flag ) {
		...
		printf( "Part  %11s %11s %4s %4s %-16s\n", "Start", "Size", "MID", "SID", "Name" );
		for ( i = 0; i < PC98_NPARTS; i++ ) {
			partp = ( (struct pc98_partition *) &mboot.parts ) + i;
			if ( partp->dp_sid == 0 ) continue;
			printf( "%4d: %11u %11u 0x%02x 0x%02x %-16.16s\n", i + 1, partp->dp_scyl * cylsecs, ( partp->dp_ecyl - partp->dp_scyl + 1 ) * cylsecs, partp->dp_mid, partp->dp_sid, partp->dp_name );
		}
		exit( 0 );
	}
	...
	if ( f_flag ) {
		...
		if ( partition == -1 ) for ( i = 1; i <= PC98_NPARTS; i++ )
			change_part( i, v_flag );
		else change_part( partition, 1 );
...
static void usage() {
	fprintf( stderr, "%s%s", "usage: fdisk [-BIaistu] [-12345678] [disk]\n", "       fdisk -f configfile [-itv] [disk]\n" );
	exit( 1 );
}
static struct pc98_partition mtpart;
static int part_unused( int i ) {
	struct pc98_partition *partp;
...
static void print_part( int i ) {
	...
	part_mb /= ( 1024 * 1024 );
	printf( "sysmid %d (%#04x),(%s)\n", partp->dp_mid, partp->dp_mid, get_type( partp->dp_mid ) );
	printf( "    start %lu, size %lu (%ju Meg), sid %d\n", ( u_long )( partp->dp_scyl * cylsecs ), (u_long) part_sz, (uintmax_t) part_mb, partp->dp_sid );
	printf( "\tbeg: cyl %d/ head %d/ sector %d;\n\tend: cyl %d/ head %d/ sector %d\n", partp->dp_scyl, partp->dp_shd, partp->dp_ssect, partp->dp_ecyl, partp->dp_ehd, partp->dp_esect );
...
static void init_sector0( unsigned long start ) {
	struct pc98_partition *partp = (struct pc98_partition *) ( &mboot.parts[0] );
	init_boot();
	partp->dp_mid = DOSMID_386BSD;
	partp->dp_sid = DOSSID_386BSD;
...
static void change_part( int i, int force ) {
	struct pc98_partition *partp = ( (struct pc98_partition *) &mboot.parts ) + i - 1;
	if ( !force && part_unused( i ) ) return;
...
static void print_params() {
	...
	if ( dos_cyls > 65535 || dos_heads > 255 || dos_sectors > 255 ) printf( "Figures below won't work with BIOS for partitions not in cyl 1\n" );
...
static void change_active( int which ) {
	...
	active = -1;
	for ( i = 0; i < PC98_NPARTS; i++ ) {
		if ( ( partp[i].dp_sid & PC98_SID_ACTIVE ) == 0 ) continue;
...
if ( active > 0 && active <= 8 ) partp[active - 1].dp_sid |= PC98_SID_ACTIVE;
}
...
static void dos( u_int32_t start, u_int32_t size, struct pc98_partition *partp ) {
u_int32_t end;
if ( partp->dp_mid == 0 && partp->dp_sid == 0 && start == 0 && size == 0 ) {
...
if ( !( st.st_mode & S_IFCHR ) ) warnx( "device %s is not character special", disk );
rwmode = I_flag || a_flag || B_flag || flag ? O_RDWR : O_RDONLY;
fd = open( disk, rwmode );
if ( fd == -1 && errno == EPERM && rwmode == O_RDWR ) fd = open( disk, O_RDONLY );
...
while ( counter < numentries ) {
	if ( ptr->type == ( type & 0x7f ) ) return ( ptr->name );
	ptr++;
	counter++;
}
return ( "unknown" );
...
if ( ( rv = regcomp( &re, "^(/dev/[a-z]+[0-9]+)([sp][0-9]+)?[a-h]?$", REG_EXTENDED ) ) != 0 ) errx( 1, "regcomp() failed (%d)", rv );
if ( ( rv = regexec( &re, rootfs.f_mntfromname, NMATCHES, rm, 0 ) ) != 0 ) errx( 1, "mounted root fs resource doesn't match expectations (regexec returned %d)", rv );
