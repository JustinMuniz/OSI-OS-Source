...
struct lump {
		off_t start;
		off_t len;
		int state;TAILQ_ENTRY(lump) list;
};
static TAILQ_HEAD(, lump) lumps = TAILQ_HEAD_INITIALIZER(lumps);
static void new_lump( off_t start, off_t len, int state ) {
	struct lump *lp;
	lp = malloc( sizeof *lp );
	if ( lp == NULL ) err( 1, "Malloc failed" );
	lp->start = start;
	lp->len = len;
	lp->state = state;
...
static void save_worklist( void ) {
	FILE *file;
	struct lump *llp;
	if ( wworklist != NULL ) {
		(void) fprintf( stderr, "\nSaving worklist ..." );
		...
		if ( file == NULL ) err( 1, "Error opening file %s", wworklist );
		TAILQ_FOREACH( llp, &lumps, list )
		fprintf( file, "%jd %jd %d\n", (intmax_t) llp->start, (intmax_t) llp->len, llp->state );
		fclose( file );
		(void) fprintf( stderr, " done.\n" );
	}
}
static off_t read_worklist( off_t t ) {
	off_t s, l, d;
	int state, lines;
	FILE *file;
	...
	if ( file == NULL ) err( 1, "Error opening file %s", rworklist );
	lines = 0;
	d = t;
	for ( ;; ) {
		++lines;
		if ( 3 != fscanf( file, "%jd %jd %d\n", &s, &l, &state ) ) {
			if ( !feof( file ) ) err( 1, "Error parsing file %s at line %d", rworklist, lines );
			else break;
		}
		new_lump( s, l, state );
		d -= l;
...
int main( int argc, char * const argv[] ) {
	...
	while ( ( ch = getopt( argc, argv, "b:r:w:s:" ) ) != -1 ) {
		switch ( ch ) {
			...
			case 'r':
				...
				if ( rworklist == NULL ) err( 1, "Cannot allocate enough memory" );
				break;
			case 's':
			...
			case 'w':
				...
				if ( wworklist == NULL ) err( 1, "Cannot allocate enough memory" );
				break;
			default:
				usage();
		}
	...
	if ( fdr < 0 ) err( 1, "Cannot open read descriptor %s", argv[0] );
	error = fstat( fdr, &sb );
	if ( error < 0 ) err( 1, "fstat failed" );
	if ( S_ISBLK( sb.st_mode ) || S_ISCHR( sb.st_mode ) ) {
		error = ioctl( fdr, DIOCGSECTORSIZE, &sectorsize );
		if ( error < 0 ) err( 1, "DIOCGSECTORSIZE failed" );
		error = ioctl( fdr, DIOCGSTRIPESIZE, &stripesize );
		if ( error == 0 && stripesize > sectorsize ) sectorsize = stripesize;
		minsize = sectorsize;
		bigsize = ( bigsize / sectorsize ) * sectorsize;
		error = ioctl( fdr, DIOCGMEDIASIZE, &t );
		if ( error < 0 ) err( 1, "DIOCGMEDIASIZE failed" );
	} else {
		t = sb.st_size;
	}
	if ( bigsize < minsize ) bigsize = minsize;
	...
	buf = malloc( bigsize );
	if ( buf == NULL ) err( 1, "Cannot allocate %zu bytes buffer", bigsize );
	if ( argc > 1 ) {
		fdw = open( argv[1], O_WRONLY | O_CREAT, DEFFILEMODE );
		if ( fdw < 0 ) err( 1, "Cannot open write descriptor %s", argv[1] );
		if ( ftruncate( fdw, t ) < 0 ) err( 1, "Cannot truncate output %s to %jd bytes", argv[1], (intmax_t) t );
	} else fdw = -1;
	if ( rworklist != NULL ) {
		d = read_worklist( t );
	} else {
	...
	if ( wworklist != NULL ) signal( SIGINT, sighandler );
	t1 = 0;
	start = len = i = state = 0;
	PRINT_HEADER;
	n = 0;
	for ( ;; ) {
		...
		while ( lp->len > 0 && !aborting ) {
			...
			if ( lp->state == 1 ) i = MIN( lp->len, (off_t) medsize );
			if ( lp->state > 1 ) i = MIN( lp->len, (off_t) minsize );
			time( &t2 );
			...
			if ( j == i ) {
				d += i;
				if ( fdw >= 0 ) j = pwrite( fdw, buf, i, lp->start );
				else j = i;
