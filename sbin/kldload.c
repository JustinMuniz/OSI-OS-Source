...
static int path_check( const char *kldname, int quiet ) {
	int mib[5], found;
	...
	if ( path == NULL ) {
		err( 1, "allocating %lu bytes for the path", (unsigned long) pathlen + 1 );
	}
		...
		if ( path_check( argv[0], quiet ) == 0 ) {
			...
			if ( fileid < 0 ) {
				if ( check_loaded != 0 && errno == EEXIST ) {
					if ( verbose ) printf( "%s is already "
							"loaded\n", argv[0] );
				} else {
				...
				if ( verbose ) printf( "Loaded %s, id=%d\n", argv[0], fileid );
			}
		} else {
			errors++;
		}
