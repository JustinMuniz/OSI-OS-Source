...
static int run_cmd( int *, const char *, ... )
__printflike(2, 3);

static enum {
	SWAPON, SWAPOFF, SWAPCTL
} orig_prog, which_prog = SWAPCTL;
...
int main( int argc, char **argv ) {
	...
	if ( strstr( ptr, "swapon" ) != NULL ) which_prog = SWAPON;
	else if ( strstr( ptr, "swapoff" ) != NULL ) which_prog = SWAPOFF;
	...
	while ( ( ch = getopt( argc, argv, "AadghklLmqsUF:" ) ) != -1 ) {
		switch ( ch ) {
			case 'A':
				...
				break;
			case 'a':
				if ( which_prog == SWAPON || which_prog == SWAPOFF ) doall = 1;
				else which_prog = SWAPON;
				break;
			case 'd':
				if ( which_prog == SWAPCTL ) which_prog = SWAPOFF;
				else usage();
				break;
			case 'g':
				hflag = 'G';
			...
			case 'm':
				...
				break;
			case 'q':
				if ( which_prog == SWAPON || which_prog == SWAPOFF ) qflag = 1;
				break;
			case 's':
				sflag = 1;
				break;
	...
	ret = 0;
	swfile = NULL;
	if ( etc_fstab != NULL ) setfstab( etc_fstab );
	if ( which_prog == SWAPON || which_prog == SWAPOFF ) {
		if ( doall ) {
			while ( ( fsp = getfsent() ) != NULL ) {
				if ( strcmp( fsp->fs_type, FSTAB_SW ) != 0 ) continue;
				if ( strstr( fsp->fs_mntops, "noauto" ) != NULL ) continue;
				
				if ( which_prog != SWAPOFF && ( strstr( fsp->fs_mntops, "late" ) != NULL || strstr( fsp->fs_mntops, "file=" ) != NULL ) && late == 0 ) continue;
				swfile = swap_on_off( fsp->fs_spec, 1, fsp->fs_mntops );
				if ( swfile == NULL ) {
					ret = 1;
					continue;
				}
				if ( qflag == 0 ) {
					printf( "%s: %sing %s as swap device\n", getprogname(), ( which_prog == SWAPOFF ) ? "remov" : "add", swfile );
				}
			}
		} else if ( *argv == NULL ) usage();
		for ( ; *argv; ++argv ) {
		...
		if ( lflag || sflag ) swaplist( lflag, sflag, hflag );
		else usage();
	}
	exit( ret );
}
...
swap_on_off_gbde( const char *name, int doingall ) {
	...
	if ( which_prog == SWAPON ) {
		...
		error = run_cmd( NULL, "%s init %s -P %s", _PATH_GBDE, dname, pass );
		if ( error ) {
			
			free( dname );
			if ( qflag == 0 ) warnx( "%s: Device already in use", name );
			return ( NULL );
		}
		error = run_cmd( NULL, "%s attach %s -p %s", _PATH_GBDE, dname, pass );
		free( dname );
		if ( error ) {
	...
	if ( which_prog == SWAPOFF ) {
		...
		if ( error ) {
			...
			if ( qflag == 0 ) warnx( "%s: Device not found", name );
			return ( NULL );
		}
	}
...
swap_on_geli_args( const char *mntops ) {
	...
	argsize = asprintf( &args, "%s%s%s%s%s%s%s%s -d", aflag, aalgo, eflag, ealgo, lflag, keylen_str, sflag, sectorsize_str );
...
static int run_cmd( int *ofd, const char *cmdline, ... ) {
	va_list ap;
	...
	switch ( pid ) {
		case 0:
			...
			if ( ofd != NULL ) if ( dup2( pfd[1], STDOUT_FILENO ) < 0 ) err( 1, "dup2 in %s", __func__ );
			nfd = open( _PATH_DEVNULL, O_RDWR );
			if ( nfd == -1 ) err( 1, "%s: open %s", __func__, _PATH_DEVNULL );
			if ( dup2( nfd, STDIN_FILENO ) < 0 ) err( 1, "%s: dup2", __func__ );
			if ( dup2dn && dup2( nfd, STDOUT_FILENO ) < 0 ) err( 1, "%s: dup2", __func__ );
			if ( dup2( nfd, STDERR_FILENO ) < 0 ) err( 1, "%s: dup2", __func__ );
			execv( argv[0], argv );
			warn( "exec: %s", argv[0] );
			_exit( -1 );
		case -1:
...
swap_on_off_sfile( const char *name, int doingall ) {
	...
	if ( which_prog == SWAPON ) error = swapon( name );
	else error = swapoff( name );
	
	if ( error == -1 ) {
		switch ( errno ) {
			case EBUSY:
				if ( doingall == 0 ) warnx( "%s: Device already in use", name );
				break;
			case EINVAL:
				if ( which_prog == SWAPON ) warnx( "%s: NSWAPDEV limit reached", name );
				else if ( doingall == 0 ) warn( "%s", name );
				break;
			default:
				warn( "%s", name );
				break;
...
static void swaplist( int lflag, int sflag, int hflag ) {
	...
	mibsize = nitems( mib );
	if ( sysctlnametomib( "vm.swap_info", mib, &mibsize ) == -1 ) err( 1, "sysctlnametomib()" );
	...
	for ( n = 0;; ++n ) {
		...
		if ( xsw.xsw_version != XSWDEV_VERSION ) errx( 1, "xswdev version mismatch" );
		...
		used += tmp_used;
		if ( lflag ) {
			sizetobuf( buf, sizeof( buf ), hflag, tmp_total, hlen, blocksize );
			printf( "/dev/%-8s %s ", devname( xsw.xsw_dev, S_IFCHR ), buf );
			sizetobuf( buf, sizeof( buf ), hflag, tmp_used, hlen, blocksize );
			printf( "%s\n", buf );
		}
	}
	if ( errno != ENOENT ) err( 1, "sysctl()" );
